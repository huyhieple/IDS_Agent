...
# Bạn nên đặt cái này ở một file config chung và import,
# hoặc đảm bảo nó khớp với label_map ở main_initialize_memory.py
...
# <<<< QUAN TRỌNG: CẬP NHẬT DANH SÁCH NÀY CHO ĐÚNG VỚI DỮ LIỆU HUẤN LUYỆN CỦA BẠN >>>>
...
# Kiểm tra lại danh sách trên, đảm bảo nó có đúng 20 features mà mô hình đã học.
...
class ClassificationTool:
    ...
    def _load_model_from_file(self, model_name: str) -> object: # Đổi tên hàm cho rõ ràng
        if model_name in self.loaded_models:
            return self.loaded_models[model_name]
        
        # Kiểm tra xem model_dir có thực sự tồn tại không
        ...
        try:
            ...
            for feature_name in EXPECTED_FEATURE_ORDER_FOR_MODELS:
                ...
                if value is None:
                    ...
                    # Việc này phải nhất quán với cách xử lý lúc huấn luyện.
    ...
    def _get_top_k_text_labels(self, model: object, probabilities: np.ndarray, k: int = 3) -> List[str]:
        ...
        if not hasattr(model, 'classes_'):
            ...
            # Fallback đơn giản nếu không có classes_ (rất hiếm cho scikit-learn classifiers)
            # Giả định probabilities đã được sắp xếp hoặc lấy top-k indices từ probabilities
        ...
        # model.classes_ thường là các số nguyên (0, 1, 2, ...) đã được map
        class_labels_from_model = model.classes_ 
        
        # Lấy top k indices từ probabilities ( probabilities là 2D array [[p0, p1, ...]])
        # argsort trả về indices để sort theo chiều tăng dần, [::-1] để đảo ngược thành giảm dần
        top_k_indices_in_proba_array = np.argsort(probabilities[0])[::-1][:k] 
        
        top_k_text_labels = []
        for index_in_proba in top_k_indices_in_proba_array:
            # index_in_proba là vị trí trong mảng probabilities (và cũng là trong model.classes_ nếu classes được sắp xếp)
            # Giá trị thực tế của lớp (ví dụ: 0, 1, 5) nằm ở class_labels_from_model[index_in_proba]
    ...
    async def predict(self, model_name: str, features_dict: Union[dict, pd.Series]) -> Dict[str, Union[str, List, None]]:
        ...
        # Chuẩn bị input features theo đúng thứ tự và số lượng
        ...
        try:
            if hasattr(model, "predict_proba"):
            ...
            else: # Xử lý mô hình không có predict_proba (proxy bạn cung cấp)
                ...
                if num_classes == 0 and hasattr(prepared_features_np, 'shape'): # Thử cách khác nếu model.classes_ không có
                    # Đây là tình huống khó, vì không biết số lớp thực sự
                    # Giả định số lớp dựa trên một cấu hình nào đó, hoặc mặc định là 2 nếu không rõ
                    ...
                    # Đoạn code proxy của bạn cần được điều chỉnh để phù hợp hơn
                    # Hoặc trả về lỗi nếu không thể tạo proxy hợp lý
                    # probabilities_list = [[0.5, 0.5]] # Ví dụ proxy rất đơn giản
                    if hasattr(model, "feature_importances_") and len(model.feature_importances_) > 0:
                        # Proxy này vẫn có vấn đề vì nó gán cùng giá trị cho tất cả "giả định" các lớp
                        # và không biết số lớp thực sự để tạo mảng đúng kích thước
                        mean_importance = np.mean(model.feature_importances_)
                        # Cần biết số lớp để tạo mảng này.
                        # Nếu model.classes_ không có, đây là một vấn đề.
                        # Tạm thời bỏ qua việc tạo proxy phức tạp nếu không có model.classes_
...
# Hàm classify để tương thích với cách gọi từ main_initialize_memory.py
async def classify(classifier_name: str, preprocessed_features_dict: dict) -> Dict[str, Union[str, List, None]]:
    ...
    """
    # Sử dụng một instance duy nhất của ClassificationTool hoặc tạo mới mỗi lần nếu cần
    # Để đơn giản, tạo mới mỗi lần ở đây (không hiệu quả nếu gọi nhiều lần liên tục)
    # Trong một hệ thống thực tế, bạn có thể muốn quản lý instance của ClassificationTool tốt hơn.
    try:
